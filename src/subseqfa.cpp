		...
		while (sam_itr_next(inFile, regionIter, alignRecord) >= 0) {
			...
			while (cigarIndex < nCigar && nextPos >= 0) {
				...
				case BAM_CDIFF:
                    ...
                    // fixed by David Gordon, March 10, 2021
                    // if (posTemplate + cigarLen > nextPos) {
                    // the problem occurred when the end of the region was
                    // reached in the last cigar element so (vague) so 
                    // doesn't reach the subEnd = ... below but exits
                    // loop because it is the last cigar element and prints
                    // too short of a # of bases
